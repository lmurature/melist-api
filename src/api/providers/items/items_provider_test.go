...
func TestConstants(t *testing.T) {
	assert.EqualValues(t, "/sites/MLA/search?q=%s", uriSearchItems)
...
func TestSearchItemsInvalidRestClientResponse(t *testing.T) {
	...
	rest.AddMockups(&rest.Mock{
		URL:          "https://api.mercadolibre.com/sites/MLA/search?q=Computadora",
		HTTPMethod:   http.MethodGet,
		RespHTTPCode: -1,
	})

	searchResult, err := SearchItemsByQuery("Computadora")
